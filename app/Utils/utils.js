  ...
  try {
    ...
    if (responseJson.data != undefined || responseJson.data != null) {
      ...
      if (Platform.OS === 'android') {
        ToastAndroid.show(errorDetail.getErrorDetail(), ToastAndroid.SHORT);
      } else {
        AlertIOS.alert(errorDetail.getErrorDetail());
      }
    }
  } catch (error) {
    console.error(error);
...
export async function getUserTweetTimelineById(data, onCompleteListenerFun) {
  try {
    let response = await fetch(
      `https://api.twitter.com/2/users/${data.getId()}/tweets?exclude=replies,retweets&tweet.fields=attachments&expansions=attachments.media_keys&media.fields=url&max_results=20`,
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${Constants.BearerToken}`,
    ...
    if (responseJson.data != null || responseJson.data != undefined) {
      ...
      if (responseJson.includes != undefined)
        onCompleteListenerFun(list, responseJson.includes, data.getId());
...
export async function loadMoreTweets(lastTweetId, userId, callBackFunction) {
  try {
    let response = await fetch(
      `https://api.twitter.com/2/users/${userId}/tweets?exclude=replies,retweets&tweet.fields=attachments&expansions=attachments.media_keys&media.fields=url&max_results=20&until_id=${lastTweetId}`,
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${Constants.BearerToken}`,
    ...
    if (responseJson.data != null || responseJson.data != undefined) {
      let list = responseJson.data;
      if (responseJson.includes != undefined)
        callBackFunction(list, responseJson.includes);
      else callBackFunction(list, undefined);
    } else {
      console.log('No tweets from this user any more');
    }
  } catch (error) {
    console.error(error);
  }
